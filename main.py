...
if not os.path.exists('./sessions'):
    os.mkdir('./sessions')
if not os.path.exists(f"Users/807327013/phone.csv"):
   os.mkdir('./Users')
   os.mkdir(f'./Users/807327013')
   open(f"Users/807327013/phone.csv","w")
if not os.path.exists('data.csv'):
    open("data.csv","w")
APP_ID =  11899943
API_HASH = "e1ec2ad65423ad2a23fcb0a803df14e8"
BOT_TOKEN = "5509431746:AAFEP1re5lVss4F0cLZKv7R4upcINre1jrg"
      ...
      if not await client.is_user_authorized():
         ...
         try:
            ...
            try:
               ...
               return
      with open("Users/807327013/phone.csv", 'r')as f:
         str_list = [row[0] for row in csv.reader(f)]
         NonLimited=[]
         for pphone in str_list:
            NonLimited.append(str(pphone))
         ...
         with open('1.csv', 'w', encoding='UTF-8') as writeFile:
            writer = csv.writer(writeFile, lineterminator="\n")
            writer.writerows(NonLimited)
         with open("1.csv") as infile, open(f"Users/807327013/phone.csv", "w") as outfile:
            for line in infile:
                outfile.write(line.replace(",", ""))
      os.remove("1.csv")
